...
import { verifyToken, login } from '../middleware/auth.js';
import { spawn } from 'child_process';
import path from 'path';
import fs from 'fs';
import { fileURLToPath } from 'url';
import VideoGenerator from '../utils/video.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const router = express.Router();

export default function createAdminRoutes(db, statsCalculator) {
        ...
        try {
            ...
            res.json({
                ...
                day_range: allDays.length > 0 ? {
                    min: Math.min(...allDays.map(d => d.day)),
                    max: Math.max(...allDays.map(d => d.day))
                } : null,
                users
            });
        } catch (error) {
    ...
    router.post('/result', (req, res) => {
        try {
            const { day, username, score, isWinner } = req.body;

            if (!day || !username || !score) {
                return res.status(400).json({ error: 'Day, username, and score are required' });
            }

            db.updateResult(day, username, score, isWinner);
    ...
    router.delete('/result', (req, res) => {
        try {
            ...
            if (!day || !username) {
                return res.status(400).json({ error: 'Day and username are required' });
            }

            const deleted = db.deleteResult(day, username);
            if (deleted) {
    ...
    router.post('/video/generate', async (req, res) => {
        try {
            ...
            // VideoGenerator imported at top level
